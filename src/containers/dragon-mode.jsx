...
import DragonTool from '../helper/tools/dragon-tool';
import DragonModeComponent from '../components/dragon-mode/dragon-mode.jsx';

class DragonMode extends React.Component {
    constructor(props) {
        super(props);
        bindAll(this, [
            'activateTool',
    ...
    componentDidMount() {
        if (this.props.isDragonModeActive) {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.isDragonModeActive && !this.props.isDragonModeActive) {
            this.activateTool();
        } else if (!nextProps.isDragonModeActive && this.props.isDragonModeActive) {
            this.deactivateTool();
        }
    }
    shouldComponentUpdate(nextProps) {
        return nextProps.isDragonModeActive !== this.props.isDragonModeActive;
    }
    ...
    render() {
        return (
            <DragonModeComponent
                isSelected={this.props.isDragonModeActive}
...
DragonMode.propTypes = {
    ...
    isDragonModeActive: PropTypes.bool.isRequired,
...
const mapStateToProps = state => ({
    ...
    isDragonModeActive: state.scratchPaint.mode === Modes.DRAGON,
...
const mapDispatchToProps = dispatch => ({
    ...
    handleMouseDown: () => {
        dispatch(changeMode(Modes.DRAGON));
    },
